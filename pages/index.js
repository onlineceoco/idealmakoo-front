...
const Own = dynamic(() =>
  import("../components/ownSlider/own")
);
      ...
      <div style={{margin:"0", padding:"0"}} ref={observe}>
        ...
        <div className={styles["container"]} ref={observe}>
          <div className={styles["threeDSection"]}>
            <React360Viewer amount={33} i={0} imagePath="images/awair-360"    fileName="t{index}.png"   autoplay={0}   loop={5}  />
          </div>
          {inView && <Dental />}
        </div>
        ...
        <div ref={observe}>{inView && <InfoSection {...homeObjectFour} />}</div>
        <div ref={observe}>{inView && <InfoSection {...homeObjectFive} />}</div>
        <World />
        {!hero && <HeroSectionLoad />}
